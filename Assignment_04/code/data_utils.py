...
def sample_points(points_xyz, points_rgb, maximum_pts_num=3000, seed=0):
    """Deterministically subsample COLMAP points to keep pure PyTorch training small."""
    n_points = points_xyz.shape[0]
    if maximum_pts_num is None or n_points <= maximum_pts_num:
        return points_xyz, points_rgb

    generator = torch.Generator(device=points_xyz.device)
    generator.manual_seed(seed)
    indices = torch.randperm(n_points, generator=generator)[:maximum_pts_num]
    indices = indices.sort().values
    return points_xyz[indices], points_rgb[indices]


class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num=3000, seed=0):
        """
        ...
        self.points3D_xyz, self.points3D_rgb = sample_points(
            self.points3D_xyz,
            self.points3D_rgb,
