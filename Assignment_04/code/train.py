...
class TrainConfig:
    ...
    history_path: str = "loss_history.csv"
    quiet: bool = False
...
class GaussianTrainer:
    ...
    def save_history(self):
        """Write per-epoch training loss to CSV."""
        history_path = Path(self.config.history_path)
        history_path.parent.mkdir(exist_ok=True, parents=True)
        with open(history_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["epoch", "loss", "seconds", "cuda_memory_mb"])
            writer.writeheader()
    ...
    def train(self, train_loader: DataLoader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            # Training loop
            pbar = tqdm(train_loader, desc=f"Epoch {epoch}", disable=self.config.quiet)
            epoch_loss = 0.0
            num_batches = 0
            ...
            if self.device.type == "cuda":
                cuda_memory_mb = torch.cuda.max_memory_allocated(self.device) / (1024 ** 2)
            self.history.append({
                "epoch": epoch,
            ...
            if self.config.quiet:
                print(
                    f"epoch={epoch:04d} "
                    f"loss={self.history[-1]['loss']:.6f} "
                    f"seconds={epoch_seconds:.3f} "
                    f"cuda_memory_mb={cuda_memory_mb:.1f}",
                    flush=True,
                )
        ...
        return {
            ...
            "peak_cuda_memory_mb": max((row["cuda_memory_mb"] for row in self.history), default=0.0),
        }
...
def parse_args():
    ...
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image downsample factor for the pure PyTorch renderer')
    parser.add_argument('--max_points', type=int, default=3000,
                      help='Maximum number of COLMAP points/Gaussians to train')
    parser.add_argument('--seed', type=int, default=0,
                      help='Random seed for point sampling and debug view selection')
    ...
    parser.add_argument('--render_video', action='store_true',
                      help='Render a side-by-side debug video after training')
    parser.add_argument('--quiet', action='store_true',
                      help='Disable tqdm and print one summary line per epoch')
...
def main():
    ...
    config = TrainConfig(
        ...
        history_path=os.path.join(args.checkpoint_dir, "loss_history.csv"),
        quiet=args.quiet,
    )
    ...
    print(f"Using {dataset.points3D_xyz.shape[0]} gaussians")
    ...
    summary_path = os.path.join(args.checkpoint_dir, "summary.txt")
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"dataset={args.colmap_dir}\n")
        f.write(f"num_images={len(dataset)}\n")
        f.write(f"num_gaussians={dataset.points3D_xyz.shape[0]}\n")
        f.write(f"num_epochs={config.num_epochs}\n")
        f.write(f"final_loss={summary['final_loss']}\n")
        f.write(f"total_seconds={summary['total_seconds']:.3f}\n")
        f.write(f"peak_cuda_memory_mb={summary['peak_cuda_memory_mb']:.3f}\n")
        f.write(f"loss_history={config.history_path}\n")
