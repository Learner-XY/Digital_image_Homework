...
if __name__ == '__main__':
    ...
    # COLMAP 3.13 moved the GPU switch from SiftExtraction to FeatureExtraction.
    subprocess.run(['colmap', 'feature_extractor', '--image_path', os.path.join(data_dir, 'images'), '--database_path', database_path, '--ImageReader.single_camera', '1', '--ImageReader.camera_model', 'PINHOLE', '--FeatureExtraction.use_gpu', '0'], check=True)

    # Feature matching
    subprocess.run(['colmap', 'exhaustive_matcher', '--database_path', database_path, '--FeatureMatching.use_gpu', '0'], check=True)
