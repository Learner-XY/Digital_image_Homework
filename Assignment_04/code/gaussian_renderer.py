...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        # 2. Get raw depths before projection for sorting and clipping
    ...
    def compute_gaussian_values(
        ...
        norm = 1.0 / (2.0 * math.pi * torch.sqrt(det))
        gaussian = norm.view(N, 1, 1) * torch.exp(exponent.clamp(min=-50.0, max=0.0))
    ...
    def forward(
        ...
        alphas = (opacities.view(N, 1, 1) * gaussian_values).clamp(0.0, 0.995)  # (N, H, W)
        ...
        transmittance = torch.cumprod(alpha_shifted, dim=0)[:-1]
