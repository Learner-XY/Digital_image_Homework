...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        if self.n_points <= 1:
            mean_dists = torch.full((self.n_points, 1), 1e-2, dtype=torch.float32)
        else:
            # Compute mean distance to nearest neighbors using pure PyTorch.
            # The first top-k entry is the point itself, so it is discarded.
            K = min(50, self.n_points - 1)
            dists = torch.cdist(points, points)
            knn_dists = torch.topk(dists, k=K + 1, dim=1, largest=False).values[:, 1:]
            mean_dists = torch.mean(knn_dists, dim=1, keepdim=True) * 2.0
